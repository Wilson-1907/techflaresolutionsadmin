...
import { getAdminApiKey, getMainSiteUrl } from "@/lib/env";
...
/** OTP login when backend supports it; fall back to password verify on older deployments. */
async function backendOtp(body: Record<string, unknown>) {
  const otp = await backendPanelAuth("/api/panel-auth/admin/login-otp", body);
  ...
  return backendPanelAuth("/api/panel-auth/admin/verify", { password: body.password });
}
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!res.ok) {
      const message = (data.error as string) || "Sign-in failed";
    ...
    const message =
      error instanceof Error && error.message.includes("ADMIN")
        ? "Admin panel is misconfigured. Set ADMIN_API_KEY and MAIN_SITE_URL on Vercel."
